...
import { GraduationCap, PlayCircle, CheckCircle2, Clock } from "lucide-react";import { createFileRoute, Link, Outlet, useLocation } from "@tanstack/react-router";
import { useQuery } from "@tanstack/react-query";
import { useMemo } from "react";
import { useStudents } from "@/lib/student-context";
import { fetchActivePrograms, fetchTodaySchedule, fetchScheduleStatuses } from "@/lib/data";
...
function ClassroomHub() {
  ...
  // Important: this route has a child route /student/classroom/$lessonId.
  // If we do not render <Outlet /> for child paths, the parent classroom hub
  // will keep showing even when the URL contains a lesson id.
  if (location.pathname !== "/student/classroom") {
    return <Outlet />;
  }
